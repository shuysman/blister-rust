...
# 3. Load climatic correlates at monitoring sites
#   - extracted from netCDF files downloaded from Daymet
#   - "NetcdfExtraction_and_SensitivityModel.R"

clim_correl_site <- read.csv("./ALL_PARKS_clim_correl_site_1980-2020.csv") %>% dplyr::filter(park != "CAN")
head(clim_correl_site);tail(clim_correl_site)
unique(clim_correl_site$park)

# 4. Merge transect-level infection data with climatic correlate data
mod_data_full <- merge(site_status, clim_correl_site, by = c("park", "transect_id"))
length(mod_data_full$transect_id)
head(mod_data_full)

# reorder mod_data
mod_data_full <- mod_data_full[with(mod_data_full, order(park, transect_id, year)),]
mod_data_full
...
mod_data <- mod_data_full %>% 
  group_by(park, transect_id) %>%
  subset(year > 1999) %>% 
  dplyr::summarize(recent_br_status = mean(recent_br_status, na.rm = TRUE),
...
# 6. Format and summarize mod_data
mod_data$period = "2000-2020"

mod_data$region <- dplyr::recode(mod_data$park,
                                 "GRYN" = "interior",
                                 "GLAC" = "interior",
                                 "SEQU" = "SW",
                                 "KICA" = "SW",
...
# check for NA
which(is.na(mod_data))

# drop one site that is missing climate data
length(mod_data$transect_id)
mod_data <- drop_na(mod_data)
length(mod_data$transect_id)
...
table(mod_data$recent_br_status, by = mod_data$park)
table(mod_data$park)
table(mod_data$recent_br_status)
length(mod_data$transect_id) # 604 transects

# 7. CACLULATE VPD -----------------------------------------------------------
mod_data$svp <- 610.8*exp((17.27*mod_data$asTEMP/(237.3+mod_data$asTEMP)))
head(mod_data)

mod_data$asRHcalc <- mod_data$asRH/100
head(mod_data)

mod_data$vpd <- (((100-mod_data$asRHcalc*100)/100)*mod_data$svp)
head(mod_data)
