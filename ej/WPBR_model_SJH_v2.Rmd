...
``` r
## Number of trees included for each "park"
monitoring_data %>%
    group_by(park) %>%
    distinct(tree_id, park) %>%
    summarize(n_trees = n()) %>%
    knitr::kable()
...
wbp_existing <- rast("/home/steve/OneDrive/whitebark/data/existing/wbp_existing.tif") %>% subst(from = 0, to = NA)

wbp_existing_poly <- terra::as.polygons(wbp_existing)
...
ggplot() +
    geom_spatraster_rgb(data = base_map, interpolate = TRUE) +
    geom_sf(data = mod_data_sf, aes(color = park), inherit.aes = FALSE) +
...
parks_sf <- st_as_sf(parks %>% filter(!is.na(lat), !is.na(long), park != "CAN", park != "GRBA", park != "ROMO", park != "GRSA"), coords = c("long", "lat"), crs = crs("EPSG:4326")) %>% st_transform(crs("EPSG:3857"))

aoi <- parks_sf %>% vect %>% buffer(width = 100000)

wbp_existing_poly_3857 <- wbp_existing_poly %>% project(crs("EPSG:3857")) %>% crop(parks_sf)

base_map <- get_tiles(aoi, provider = "Esri.WorldStreetMap", crop = TRUE, zoom = 5)

ggplot() +
    geom_spatraster_rgb(data = base_map, maxcell = Inf, interpolate = TRUE) +
    geom_sf(data = wbp_existing_poly_3857, alpha = 0.5, fill = "darkgray", inherit.aes = FALSE) +
    geom_sf(data = parks_sf, aes(color = park), size = 1, inherit.aes = FALSE) +
    coord_sf(crs = 3857) +
    ggtitle("PIAL WPBR Monitoring Transects") +
    labs(color = "Park") +
    scale_x_continuous(labels = function(x) paste0(x)) +
    scale_y_continuous(labels = function(x) paste0(x))
##ggsave("wpbr_map.png", width = 8, height = 8)
...
```{r}
cols = c("br_status", "asT", "asVPDmean", "asRH", "asP", "dbh_cm", "log_dbh_cm")

ggpairs(mod_data_tree_level, columns = cols)

##ggsave("Report/pairs.png", width = 6, height = 6)

ggpairs(mod_data_tree_level, mapping = aes(color = park), columns = cols)


## "Fivepeaks" network seems to be overcontributing to some variables compared to other monitoring locations
## There is spatial autocorrelation in the data?
mod_data_tree_level %>%
    pivot_longer(cols = c(asVPDmean, asT, asRH, asP)) %>%
...
## Test for Covariance
```{r}
glmer_all <- glmer(br_status ~ asT + asVPDmean + asRH + asP + dbh_cm + (1 | park) + (1 | park:transect_id),
...
library(corrplot)
corrplot(cor(select(mod_data_tree_level, asT, asVPDmean, asP, asRH, dbh_cm)), method = "number", type = "upper")
...
glm_vpd_p_park <- glm(br_status ~ asVPDmean* asP + dbh_cm + park,
                   data = mod_data_tree_level,
                   family = binomial
                 )
summary(glm_vpd_p_park)

glm_vpd_2_p_2_park <- glm(br_status ~ poly(asVPDmean, 2) * poly(asP, 2) + dbh_cm + park,
                   data = mod_data_tree_level,
                   family = binomial
                 )
...
png("Report/t_semivario.png")
plot(t_semivario, main = "Temperature Variogram")
dev.off()

vpd_semivario <- variogram(asVPDmean ~ 1, data = mod_data_tree_level, locations = ~X+Y)
png("Report/vpd_semivario.png")
plot(vpd_semivario, main = "VPD Variogram")
dev.off()

rh_semivario <- variogram(asRH ~ 1, data = mod_data_tree_level, locations = ~X+Y)
png("Report/rh_semivario.png")
plot(rh_semivario, main = "RH Variogram")
dev.off()

p_semivario <- variogram(asP ~ 1, data = mod_data_tree_level, locations = ~X+Y)
png("Report/p_semivario.png")
plot(p_semivario, main = "Precipitation Variogram")
dev.off()
...
##     #geom_sf(data = wbp_existing_poly_3857, alpha = 0.5, fill = "darkgray", inherit.aes = FALSE) +
##     #geom_sf(data = parks_sf, aes(color = park), size = 1, inherit.aes = FALSE) +
##     inlabru::gg(mesh$mesh, crs = sf::st_crs(3857)) +
##     coord_sf(crs = 3857) +
##     ggtitle("PIAL WPBR Monitoring Transects") +
##     labs(color = "Park") +
##     scale_x_continuous(labels = function(x) paste0(x)) +
...
plot(ggeffect(sdm_t_2_x_p), facet = TRUE, add.data = TRUE, ci = FALSE) + xlab("") + ylab("P(Infection)")

plot(ggeffect(sdm_t_2_x_p, terms = c("asT", "asP[25:175 by = 50]")), add.data = TRUE)


df <- ggeffect(sdm_t_2_x_p, terms = c("asP", "asT [8:14 by=2]"))
...
pr <- ggpredict(sdm_t_2_x_p, c("asP", "asT"))


plot(ggeffect(sdm_t_2_x_p, terms = c("asP", "asT [8:14 by=2]")), add.data = TRUE, ci = FALSE) + ggtitle("Temperature x Precipitation Interaction") + xlab("Aug/Sep Precipitation") + ylab("P(Infection)") + labs(color = "Aug/Sep Temperature") + theme(legend.position = "bottom")
...
ggsave("Report/t_2_x_p_confusion_matrix.png")
